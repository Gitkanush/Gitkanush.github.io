...
' en la hoja Determinador y poder revisar con mayor facilidad los productos
...
Private Sub cbGrupo_Change()
    ...
    If cbGrupo.Value = "Ninguno" Then
        ...
        Determinador.ListObjects("tDeterminador").Range.AutoFilter
    Else
        Determinador.cbSubgrupo.Clear
        With Separador.ListObjects("tSeparadores")
            For Each fila In .ListRows
                If .DataBodyRange(fila.Index, .ListColumns("Grupo").Index).Value = cbGrupo.Value Then
                    Determinador.cbSubgrupo.AddItem .DataBodyRange(fila.Index, .ListColumns("SubGrupo").Index).Value
                End If
            Next fila
        End With
        ...
        With Datos.ListObjects("tDatos")
            ...
            Next fila
            Determinador.Unprotect
            Determinador.ListObjects("tDeterminador").Range.AutoFilter 1, lista, xlFilterValues
        End With
    End If
handler:
...
Private Sub cbSubgrupo_Change()
    ...
    Else
        With Separador.ListObjects("tSeparadores")
            ...
            Next fila
        End With
        Determinador.ListObjects("tDeterminador").Range.AutoFilter Field:=1, Criteria1:=">=" & inicio, Operator:=xlAnd, Criteria2:="<=" & final
    End If
handler:
    Determinador.Protect AllowFiltering:=True
End Sub
        ...
        With Separador.ListObjects("tSeparadores")
            ...
            For Each fila In .ListRows
                ...
                ElseIf .DataBodyRange(fila.Index, .ListColumns("Grupo").Index) <> .DataBodyRange(fila.Index - 1, .ListColumns("Grupo").Index) Then
                    If .DataBodyRange(fila.Index, .ListColumns("Grupo").Index) = "51-FERRETERIA" And .DataBodyRange(fila.Index, .ListColumns("Grupo").Index) = "61-PISOS-RECUB-ACC" Then
                        Determinador.cbGrupo.AddItem .DataBodyRange(fila.Index, .ListColumns("Grupo").Index).Value
                    End If
                End If
            Next fila
